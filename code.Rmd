...
The variable *amenities* contains all the amenities of the listing. However, this was stored in one large string, and the elements could not be accessed separately. Therefore, we cleaned the string and spitted it, so we had a list containing the separated elements. Furthermore, we extracted all unique amenities and stored these in a vector. 
...
For the variable *host_verification* the same applies as to *amenities*. The same cleaning method was used as for amenities, first we cleaned and splitted the strings, thereafter a vector with all unique amenities is generated.
...
A plot for finding the best mtry, based on the criteria of the mae. A lower mae would indicate a better result, as a lower value indicates a lower error of prediction. 
```{r Plot MAE}
