...
2. Mean absolute error. The objective is to minimize the result of this metric. This metrics is more robust and therefore less sensitive to observations with large absolute residuals. However, because it uses the mean and therefore it is not insensitive to skewed distributions. 
3. R-squared. An attraction is that the metric is unitless and can therefore be compared across models. A downside is that is not robust, since it measures essential correlation and not agreement. 
