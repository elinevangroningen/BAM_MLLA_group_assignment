...
The variable *amenities* contains all the amenities of the listing. However, this was stored in one large string, and the elements could not be accessed separately. Therefore, we cleaned the string and split it to a list containing the separated elements. Furthermore, we extracted all unique amenities and stored these in a vector. 
```{r}
...
As the following section will show, this is an non-optimal result compared to not including the binary variables (see part 3). Therefore, it is chosen to remove all the categorical variables (implying the binary variables as well) from the KNN-model. Moreover, it should be kept in mind that how well a KNN-model works can decline when too many feature are added to the model. These points are all kept in mind in specifying the recipe for the KNN-model.
```{r}
# Specify model 
knn_mod <- 
